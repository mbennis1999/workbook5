...
//set up our access to out elements on the html document
const selectDiv = document.querySelector("#select-div");
 //const selectdivById = document.getElementById("select-div");

window.onload = function () {
  createDropDownMenu(categories);

  //the display function will need to be chnaged because it will need to get its 
  //alue from the select element
  //otjer should't be  hardcoded
  selectDiv.addEventListener("change", displayActivityByCategory("Other"));
}


//create a function that will create a drop down menu
function createDropDownMenu(categories) {
  //all we want to do is create the drop down menu
  ...
  for( let i = 0; i <= categories.length; i++){
    ...
    if(i === 0){
      ...
      //if its the very beginning of the array, i want to add
      //'Select image...'
      dropDown.appendChild(populateDropDownMenu("Select category..."));

    } else {

      //else -  i want to add the category name

      dropDown.appendChild(populateDropDownMenu(categories[i-1]));

    }

    //this needs to go on a onchange
    // displayActivityByCategory(categories[i]);
...
function populateDropDownMenu(category){
...
function displayActivityByCategory(category){
